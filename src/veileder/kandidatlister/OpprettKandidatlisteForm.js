    ...
    getEmployerSuggestionLabel = (suggestion) => {
        ...
        if (suggestion.location) {
            ...
            if (suggestion.location.city) {
                commaSeparate = [...commaSeparate, this.capitalizeLocation(suggestion.location.city)];
            }
        }
        if (suggestion.orgnr) {
...
const mapDispatchToProps = (dispatch) => ({
    fetchTypeaheadSuggestions: (value) => dispatch({ type: FETCH_TYPE_AHEAD_SUGGESTIONS_ENHETSREGISTER, value }),
    clearTypeaheadSuggestions: () => dispatch({ type: CLEAR_TYPE_AHEAD_SUGGESTIONS_ENHETSREGISTER })
});
