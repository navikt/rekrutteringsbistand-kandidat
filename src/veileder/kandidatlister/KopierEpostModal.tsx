...
const KopierEpostModal : FunctionComponent<Props> = ({ vis=true, onClose, kandidater }) => {
    ...
    const onKopierEpostadresser = () => {
        const epostStreng = kandidaterMedEpost
        ...
        setSuccessMelding(
            <div>
                ...
                <Normaltekst>
                    Lim inn adressene i mailprogrammet, og bruk blindkopi (Bcc) feltet.
                    De som mottar e-posten vil da kun se sin egen adresse.
                </Normaltekst>
            </div>
        );
    };
    return (
        ...
        >
            ...
            <Ingress className="SendEpostModal-ingress">
                <span className="bold">{kandidaterMedEpost.length} </span> av <span className="bold">{kandidater.length}</span> markerte kandidater har registrert e-postadresse
            </Ingress>
            <div className="kandidat-tabell">
                <div className="kandidat-tabell-rad">
                    <Element tag="div" className="kandidat-tabell-rad-navn">Navn</Element>
                    <Element tag="div" className="kandidat-tabell-rad-epost">E-postadresse</Element>
                </div>
                <div className="kandidat-tabell-innhold">
                    {kandidater.map((kandidat) => (
                        <div className={`kandidat-tabell-rad ${kandidat.epost ? '' : 'kandidat-tabell-rad__inaktiv'}`} key={kandidat.kandidatId} >
                            <div className="kandidat-tabell-rad-navn">
                                <span>{`${kandidat.fornavn ? capitalizeFirstLetter(kandidat.fornavn) : ''} ${kandidat.etternavn ? capitalizeFirstLetter(kandidat.etternavn) : ''}`}</span>
                            </div>
                            <div className="kandidat-tabell-rad-epost">
                                {kandidat.epost
                                    ? <Lenke className="link" href={`mailto:${kandidat.epost}`}>{kandidat.epost}</Lenke>
                                    : <span>&ndash;</span>}
                            </div>
                        </div>
                    ))}
            ...
            <div className="SendEpostModal--knapperad">
                <Knapp onClick={onKopierEpostadresser} disabled={kandidaterMedEpost.length === 0}>Kopier e-postadresser</Knapp>
